...
testBasic = TestCase $ do
  let func_acl = AccessControlList [RoleAccess { role = role1,
                                                 perms = S.singleton ViewFunctionPermission }]
      role1 = fakeUUID 1
      role2 = fakeUUID 2

  assertBool "simple hasAccess" (hasAccess [role1] ViewFunctionPermission func_acl)

  let func_acl1 = addAccess role1 ExecuteFunctionPermission func_acl
  assertBool "addAccess to existing role" (hasAccess [role1] ExecuteFunctionPermission func_acl1)

  let func_acl2 = addAccess role2 ViewFunctionPermission func_acl
  ...
  let func_acl4 = addAccess role2 ExecuteFunctionPermission func_acl
