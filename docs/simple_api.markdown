...
For common use cases, the ```ProjectM36.Client.Simple``` API can assist in transaction management. The API is designed to be less overwhelming than the full-featured ```ProjectM36.Client```.
...
1. Create a ```DbConn``` to connect to the database:
 ...
 ...
 eDbconn <- simpleConnectProjectM36 (CrashSafePersistence (tempdir </> "db")) emptyNotificationCallback []
 ```

 The result is either a ```DbError``` or the connection linked to the "master" branch of the database.

1. Construct the ```Db``` monad using ```withTransaction```:
